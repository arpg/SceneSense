...
def key_callback(vis):
    ...
    #rotate the coor frame so it looks normal
    homogeneous_matrix = np.eye(4)
    ...
    homogeneous_matrix[:3, :3] = rotation_matrix
    # homogeneous_matrix[:3, 3] = translation
    coor.transform(hm_tx_mat@homogeneous_matrix)
    ...
    pruned_diff_points[:,0] = pruned_diff_points[:,0] + 0.03
    pruned_diff_points[:,1] = pruned_diff_points[:,1] + -0.04
    #create pruned pcd
    pruned_diff_pcd = o3d.geometry.PointCloud()
    pruned_diff_pcd.points = o3d.utility.Vector3dVector(pruned_diff_points)
    pruned_diff_pcd.transform(hm_tx_mat)
    #prune gt data
    #lets go through tall the dif fpoints and remove anything that is super close to a local point
    ...
    #show the associated RGB image
    img_path = "/hdd/sceneDiff_data/house_2/step_" + str(step) + "/running_octomap/rgb_.png" 
