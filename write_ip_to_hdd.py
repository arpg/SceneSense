...
for step in house_dirs:
    ...
    for i, img in enumerate(local_octomap_pm):
        ...
        cv2.imwrite(house_path + step + "/fid_data/gt/" + str(i) + ".png", output )
    ...
    for i, img in enumerate(local_octomap_pm):
        ...
        output = np.repeat(output[:, :, np.newaxis], 3, axis=2)
        #save it as a cv2 image
        cv2.imwrite(house_path + step + "/fid_data/predicted/" + str(i) + ".png", output )
    # #compute the fid
    fid_val = fid.compute_fid(house_path + step + "/fid_data/predicted/", house_path + step + "/fid_data/gt/")
    np.savetxt(house_path + step + "/fid_val.txt", np.array([fid_val]))
    print("fid: ", fid_val)
    #compute kid
    kid_val = fid.compute_kid(house_path + step + "/fid_data/predicted/", house_path + step + "/fid_data/gt/")
    np.savetxt(house_path + step + "/kid_val.txt", np.array([kid_val]))
    print("kid: ", kid_val)
