...
#remove the celing
local_gt_points = local_gt_points[local_gt_points[:,1] < 2]
...
# local_pcd_points = local_pcd_points[local_pcd_points[:,1] < 1.5]
...
#                                          x_y_bounds = [-2, 2],
#                                           z_bounds = [-1.4, 0.9])
# noise_pcd.points = o3d.utility.Vector3dVector(inpained_points)
# noise_pcd.rotate(R, center=(0, 0, 0))
# o3d.visualization.draw_geometries([noise_pcd])

###################### show the inpainted noise
...
# #remove the lower floors
# local_octomap_points = local_octomap_points[local_octomap_points[:,1] > -0.5]
# #remove the celing
...
#                                          x_y_bounds = [-2, 2],
#                                           z_bounds = [-1.4, 0.9])
# # returned_pc = returned_pc[returned_pc[:,1] > -0.5]
# # #remove the celing
# returned_pc = returned_pc[returned_pc[:,1] < 0.8]
...
# # now just need to map unoc and occ region into the 
# unnoc_grid = torch.tensor(unoc_pm, dtype = torch.float)
# unnoc_coordinate = np.where(unnoc_grid > 0.9)
# voxel_grid = torch.tensor(local_octomap_pm, dtype=torch.float)
# #get the coordinates of the occupied voxels
# input_coordinate = np.where(voxel_grid > 0.9)

# #add noise to the grids
# noise_scheduler = DDPMScheduler(num_train_timesteps=1000)
# noise_scheduler.set_timesteps(20)
# noise_for_grid = torch.randn((23,40,40))
# for t in tqdm(noise_scheduler.timesteps):
#     # unnoc_grid = noise_scheduler.add_noise(unnoc_grid, noise_for_grid, timesteps = torch.tensor([t.item()]))
...
#                 noise[z_val, x_val, y_val] = 1 - unnoc_grid[z_val, x_val, y_val]
#             # EXPECT THESE 
...
#                 # print(z_val, x_val, y_val)
#                 #replace the latent value with the new noisified input value
#                 noise[z_val, x_val, y_val] = voxel_grid[z_val, x_val, y_val]
#     noise_pcd = o3d.geometry.PointCloud()
#     inpained_points = utils.pointmap_to_pc(noise,
#                                             voxel_size = 0.1,
#                                             x_y_bounds = [-2, 2],
#                                             z_bounds = [-1.4, 0.9])
#     noise_pcd.points = o3d.utility.Vector3dVector(inpained_points)
