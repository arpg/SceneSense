...
for coor_idx in list(range(len(running_occ_files)))[402:450]:
    ...
    if len(os.listdir(running_occ_path + running_occ_files[coor_idx])) != 0:
        ...
        diffused_pcd = o3d.io.read_point_cloud("/hdd/sceneDiff_data/real_data/diff_pcds/" + str(correct_nums) + "p_c.pcd")
        colors = np.zeros((len(np.asarray(local_running_occ_pcd.points)), 3))
        max_z = np.max(np.asarray(local_running_occ_pcd.points)[:,1], axis = 0)
        min_z = np.min(np.asarray(local_running_occ_pcd.points)[:,1], axis = 0)
        colors[:,0] = ((np.asarray(local_running_occ_pcd.points)[:,1] - min_z)/(max_z - min_z))*(1 - 0.3) + 0.3
        colors[:,1] = ((np.asarray(local_running_occ_pcd.points)[:,1] - min_z)/(max_z - min_z))*(1 - 0.3) + 0.3
        local_running_occ_pcd.colors = o3d.utility.Vector3dVector(colors)
        o3d.visualization.draw_geometries([local_running_occ_pcd])
        o3d.visualization.draw_geometries([diffused_pcd])
