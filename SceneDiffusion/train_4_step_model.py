...
for epoch in range(config.num_epochs):
    ...
    for (step, batch), conditioning_batch in zip(enumerate(gt_dataloader),conditioning_dataloader):
        ...
        #append the conditioning together to make multiframe conditioning
        #initialize 
        stacked_cond = post_model_conditioning_batch[0]
        for j in range(1,4):
            stacked_cond = torch.cat((stacked_cond,post_model_conditioning_batch[j]), 0)
        
        stacked_cond = stacked_cond[None,:,:]
        for idx in range(1,post_model_conditioning_batch.shape[0] - 3):
            running_stack = post_model_conditioning_batch[idx]
            for j in range(1,4):
                running_stack = torch.cat((running_stack,post_model_conditioning_batch[idx + j]), 0)
            running_stack = running_stack[None,:,:]
            stacked_cond = torch.cat((stacked_cond,running_stack), 0)
        print("stacked cond shape: ", stacked_cond.shape)
        
        #and reduce total number of target frames  
        # noisy_images = noisy_images[3:]     
        print("noisey images: ", noisy_images.shape)
        # Predict the noise residual
        #compute if one of the conditioning batches should be set to zeros
        n = random.sample(range(0,100), len(stacked_cond))
        dropout_idx = [i for i,v in enumerate(n) if v < dropout*100]

        zeroed_conditioning = np.zeros((stacked_cond.shape[1],stacked_cond.shape[2]), dtype = np.single)
        ...
        for i in dropout_idx:
            stacked_cond[i] = zeroed_conditioning           

        # print(noisy_images.dtype)
        # print(post_model_conditioning_batch.dtype)
        print(noisy_images.shape)
        print(stacked_cond.shape)
        noise_pred = model(noisy_images, timesteps, encoder_hidden_states=stacked_cond, return_dict=False)[0]
    ...
    print("\nPushing to Hub\n")
    model.push_to_hub("rgbd_4_frame_unet")
    torch.save(conditioning_model.state_dict(), "/home/arpg/Documents/SceneDiffusion/cond_weights_4_frame/cond_model" + str(epoch))
