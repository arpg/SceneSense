...
from SceneDiffusion.pointnet2_scene_diffusion import get_model
import os
...
if __name__ == '__main__':
    ...
    #get gt data
    #########################3
    gt_dir = "/home/brendan/spot_data/dataset/trajs/IRL/ex_gt_point_maps/"
    ...
    gt_data = gt_data.astype(np.single)[1:]
    print(gt_data.shape)
    ...
    cond_dir = "/home/brendan/spot_data/dataset/trajs/IRL/ex_cond/"
    cond_files = natsorted(os.listdir(cond_dir))

    #initalize gt data cube:
    max_cond = np.max([np.load(cond_dir+cond_file).shape[0] for cond_file in cond_files])
    cond_data = np.zeros(( max_cond, 3))
    #get into correct shape
    print(cond_data.shape)
    cond_data = np.transpose(cond_data)
    #add batch dim
    cond_data = cond_data[None,:,:]
    print(cond_data.shape)
    
    for cond_file in cond_files:
        single_cond_data_ = np.zeros((1, 3, max_cond))
        single_cond_data = np.load(cond_dir + cond_file)
        #get into correct shape
        single_cond_data = np.transpose(single_cond_data)
        #add batch dim
        single_cond_data = single_cond_data[None,:,:]
        single_cond_data_[:, :, :single_cond_data.shape[2]]
        #append to data cube
        cond_data = np.append(cond_data, single_cond_data_, axis = 0)

    cond_data = cond_data.astype(np.single)[1:]
    print(cond_data.shape)
    ...
    conditioning_dataloader = torch.utils.data.DataLoader(cond_data, batch_size=config.train_batch_size, shuffle=False)
