...
print(
    "Unet output shape:",
    model(
        sample_noise_start, timestep=1.0, encoder_hidden_states=sample_conditioning
    ).sample.shape,
)
...
for i in range(len(cond_files)):
    ...
    if split_gt_files[0] == split_conditioning_files[0] and int(
        split_gt_files[1]
    ) == int(split_conditioning_files[2]):
...
gt_dataloader = torch.utils.data.DataLoader(
    aligned_gt_files, batch_size=config.train_batch_size, shuffle=False
)
conditioning_dataloader = torch.utils.data.DataLoader(
    aligned_cond_files, batch_size=config.train_batch_size, shuffle=False
)
...
optimizer = torch.optim.AdamW(
    list(model.parameters()) + list(conditioning_model.parameters()),
...
timesteps = torch.randint(
    ...
    (bs,),
    device=sample_noise_start.device,
).long()

global_step = 0
dropout = 0.1
# Now you train the model
zeroed_conditioning = np.zeros(
    (sample_conditioning.shape[1], sample_conditioning.shape[2]), dtype=np.single
)
...
for epoch in range(config.num_epochs):
    ...
    for (step, batch), conditioning_batch in zip(
        enumerate(gt_dataloader), conditioning_dataloader
    ):
        #####################################################
        # load the gt data
        ####################################################
        ...
        cond_tensor = conditioning_model(
            torch.tensor(cond_data.astype(np.single)).to(torch_device)
        )
        ...
        for cond_file in conditioning_batch[1:]:
            ...
            single_cond_tensor = conditioning_model(
                torch.tensor(single_cond_data.astype(np.single)).to(torch_device)
            )
            # append to data cube
        ...
        noise_pred = model(
            noisy_images,
            timesteps,
            encoder_hidden_states=post_model_conditioning_batch,
            return_dict=False,
        )[0]
    ...
    torch.save(
        conditioning_model.state_dict(),
        "full_sim_pointnet_weights_more_pointnet_short/" + str(epoch),
    )
