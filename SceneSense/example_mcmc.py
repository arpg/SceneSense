...
import SceneSense.utils.mcmc_utils as utils
from SceneSense.utils.pointnet2_scene_diffusion import get_model
from SceneSense.utils.utils import pc_to_pointmap, pointmap_to_pc
...
# create random vecotr for conditoning data
pointnet_conditioing = torch.randn((1, 2048, 128))
...
noise_scheduler = DDPMScheduler(num_train_timesteps=1000)
print("after noise sched")
# get the local pointmap
local_octomap_points = points_within_distance(
    curr_coor[0], curr_coor[2], np.asarray(pcd.points), 2.0
)
print("blaj")
...
print("afrte deep copy")
zeroed_local_pcd.transform(utils.inverse_homogeneous_transform(hm_tx_mat))
# turn the points into a pointmap
print("before pointmap")
local_octomap_pm = pc_to_pointmap(
    np.asarray(zeroed_local_pcd.points),
    voxel_size=0.1,
    x_y_bounds=[-2.0, 2.0],
    z_bounds=[-1.4, 0.9],
)
print("after pointmap")
returned_pc = pointmap_to_pc(
    pointmap=local_octomap_pm, voxel_size=0.1, x_y_bounds=[-2, 2], z_bounds=[-1.4, 0.9]
)
...
unoc_pm = pc_to_pointmap(
    unoc_local_points, voxel_size=0.1, x_y_bounds=[-2.0, 2.0], z_bounds=[-1.4, 0.9]
)
returned_unoc_pc = pointmap_to_pc(
    pointmap=unoc_pm, voxel_size=0.1, x_y_bounds=[-2, 2], z_bounds=[-1.4, 0.9]
)
...
inpained_points = pointmap_to_pc(
